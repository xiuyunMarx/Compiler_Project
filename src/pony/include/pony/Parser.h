 ...
 // Parse a variable declaration, decl ::= var identifier [ type ] = expr
  ...
  std::unique_ptr<VarType> parseType() {
    ...
    if (lexer.getCurToken() == tok_sbracket_open){
      ...
      while (lexer.getCurToken() == tok_sbracket_open) {
        lexer.getNextToken(); // eat '['

        // Must be a number
        if (lexer.getCurToken() != tok_number)
          return parseError<VarType>("number", "Expected integer dimension inside '['");

        auto dim = lexer.getValue();
        lexer.getNextToken(); // eat number
        type->shape.push_back(dim);

        // Must follow with a ']'
        if (lexer.getCurToken() != tok_sbracket_close)
          return parseError<VarType>("]", "Expected ']' after dimension");
        lexer.getNextToken(); // eat ']'
      }
      return type;
    } else if (lexer.getCurToken() == '<') {
  ...
  std::unique_ptr<ExprAST> parseIdentifierExpr() {
    ...
    assert(lexer.getCurToken() == tok_identifier);
    ...
    if (lexer.getCurToken() != tok_parenthese_close) {
      while (true) {
        ...
        if (lexer.getCurToken() != tok_comma)
          return parseError<ExprAST>(" ) or ," , "To close function call or to separate arguments");
        lexer.getNextToken();  // eat ','
      }
    }
    ...
    if (idName == "print") {
      if (args.size() != 1)
        return parseError<ExprAST>("only 1 argument", "in print Call");
