...
private:
  ...
  std::unique_ptr<VarDeclExprAST> parseDeclaration() {
    ...
    llvm::StringRef id = lexer.getId();
    lexer.consume(tok_identifier);

    // Parse optional type: <shape> or [dim][dim]...
    ...
    if (lexer.getCurToken() == '<') {
      ...
      while (lexer.getCurToken() == '[') {
        ...
        if (lexer.getCurToken() != tok_number)
          return parseError<VarDeclExprAST>("number", "in array shape");
        type->shape.push_back(lexer.getValue());
        lexer.getNextToken(); // eat number
        if (lexer.getCurToken() != ']')
          return parseError<VarDeclExprAST>("]", "to close array shape");
        lexer.getNextToken(); // eat ']'
      }
    }
    if (!type)
    ...
    lexer.consume(Token('='));
    auto expr = parseExpression();
    return std::make_unique<VarDeclExprAST>(std::move(loc), id, std::move(*type), std::move(expr));
  }
  ...
  std::unique_ptr<ExprAST> parseIdentifierExpr() {
    auto loc = lexer.getLastLocation();
    llvm::StringRef name = lexer.getId();
    lexer.consume(tok_identifier);
    // Simple variable
    if (lexer.getCurToken() != '(')
      return std::make_unique<VariableExprAST>(loc, name);
    // Function or print call
    lexer.consume(Token('('));
    ...
    if (name == "print") {
      if (args.size() != 1)
        return parseError<ExprAST>("single argument", "in print");
  ...
  int getTokPrecedence() {
    ...
    case '+':
      return 20;
    case '*':
    case '@': // matrix multiplication with same precedence as '*'
      return 40;
    default:
      return -1;
    }
  ...
  std::unique_ptr<ExprAST> parseBinOpRHS(int exprPrec,
    ...
    while (true) {
      ...
      if (tokPrec < exprPrec)
        return lhs;
      char op = static_cast<char>(lexer.getCurToken());
      auto opLoc = lexer.getLastLocation();
      lexer.getNextToken(); // eat operator

      // Parse the primary expression after operator
      auto rhs = parsePrimary();
      if (!rhs) return nullptr;

      // If next operator has higher precedence, parse recursively
      ...
      // Merge lhs and rhs
      lhs = std::make_unique<BinaryExprAST>(std::move(opLoc), op,
